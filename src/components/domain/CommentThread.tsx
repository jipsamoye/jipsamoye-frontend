...
/** TComment 에서 렌더에 필요한 필드 */
export interface RenderableComment extends BaseThreadedComment {
  nickname: string;
  profileImageUrl: string | null;
  authorTotalLikeCount: number;
  mentionedNickname: string | null;
  createdAt: string;
  replies?: RenderableComment[];
}

export interface CommentThreadProps<TComment extends RenderableComment> {
  // 훅 결과
  comments: TComment[];
  hasNext: boolean;
  loading: boolean;
  ...
  updateComment: (id: number, content: string) => Promise<TComment>;
  ...
  /**
   * 댓글 입력 maxLength.
   * undefined 이면 maxLength 속성을 렌더하지 않는다.
   */
  commentMaxLength?: number;

  /**
   * true 이면 아바타를 hidden md:block 래퍼로 감싼다 (모바일 숨김).
   * false 이면 아바타를 직접 렌더한다.
   */
  hideAvatarOnMobile: boolean;
}

export default function CommentThread<TComment extends RenderableComment>({
  comments,
  hasNext,
  loading,
  loadMore,
  ...
  const renderContent = (c: RenderableComment, isReply: boolean) => {
  ...
  const renderAvatar = (src: string | null, size: 'md' | 'xs' = 'md') => {
    if (hideAvatarOnMobile) {
      return (
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 mt-6 pt-5 border-t border-gray-100">
        {user ? (
          <>
            {renderAvatar(user.profileImageUrl)}
            <input
              value={commentText}
              ...
              className={`${hideAvatarOnMobile ? 'flex-1 min-w-0' : 'flex-1'} bg-[#F2F2F2] rounded-xl py-3.5 px-5 text-sm outline-none placeholder:text-[#B0B0B0]`}
            />
            <button
              onClick={handleAddParent}
              className={`${hideAvatarOnMobile ? 'flex-shrink-0 ' : ''}bg-[#1A1A1A] text-white text-sm font-semibold rounded-xl py-3.5 px-5 whitespace-nowrap hover:bg-[#333] transition-colors`}
            >
              댓글
            </button>
          </>
          ...
          <div
            className={`flex items-center gap-3 ${hideAvatarOnMobile ? 'flex-1 min-w-0' : 'flex-1'} cursor-pointer`}
          ...
          >
            ...
            <div className={`${hideAvatarOnMobile ? 'flex-1 min-w-0' : 'flex-1'} bg-[#F2F2F2] rounded-xl py-3.5 px-5 text-sm text-[#B0B0B0] select-none`}>
              회원만 댓글 달 수 있어요!
            </div>
            <div className={`${hideAvatarOnMobile ? 'flex-shrink-0 ' : ''}bg-[#D4D4D4] text-[#ABABAB] text-sm font-semibold rounded-xl py-3.5 px-5 whitespace-nowrap`}>
              댓글
            </div>
          </div>
        )}
