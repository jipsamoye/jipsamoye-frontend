...
describe('FigurineCreator', () => {
  ...
  it('상단 설명이 키캡 문구로 노출되고 핵심 구절은 amber로 강조된다', () => {
    render(<FigurineCreator />);
    const highlight = screen.getByText('키캡 위 미니 피규어');
  ...
  it('비로그인 상태에서 드롭 시 파일을 받지 않고 로그인 모달을 띄운다', () => {
    authMock.user = null;
    render(<FigurineCreator />);
    const dropzone = screen.getByText('JPG / PNG / WEBP').closest('div[class*="border-dashed"]');

    fireEvent.drop(dropzone!, {
      dataTransfer: { files: [new File(['x'], 'cat.jpg', { type: 'image/jpeg' })] },
    });
