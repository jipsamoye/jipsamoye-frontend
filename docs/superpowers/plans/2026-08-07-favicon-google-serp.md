...
2. `assertGlyphCoverage` — 한글 폰트가 없는 환경에서 구우면 글자가 사라지거나 두부(□)가 된다. `document.fonts.check()`는 **가짜 폰트명에도 `true`를 반환해 가드로 쓸 수 없음이 실측으로 확인됐다.** 대신 렌더 결과의 흰색(글자색) 픽셀 비율을 본다. 실측 정상값은 20.5~25.8%이므로 10~40% 밖이면 실패시킨다.
   > 정직하게 적어둘 한계: 이 가드는 "글자가 아예 안 그려짐"과 "전부 흰색"은 잡지만, **두부(□□) 감지는 best-effort라 100% 잡아내지는 못한다.** `git diff`는 바이너리 산출물(.ico/.png)에 대해 "Binary files differ"만 보여줄 뿐 내용을 보여주지 않으므로 방어선이 되지 못한다 — 최종 방어선은 **사람이 생성된 PNG를 직접 열어 눈으로 확인하는 것**이다.
...
계획 실행 중/후에 확정된 사항. 위 본문의 계획 시점 기술과 다르면 **이 절이 최신**이다.

1. **ICO는 16을 빼고 32/48 두 사이즈로 확정.** 계획은 16/32/48이었으나, 굽고 실물을 확인한 결과
   한글 4글자를 16px로 직접 렌더하면 판독 불가한 얼룩이 되어 사용자가 16 제외를 결정했다.
   구글은 48을, 2× 디스플레이 브라우저는 32를 쓰고, 16이 필요한 소비자는 32/48을 다운스케일하는데
   그쪽이 직접 렌더보다 결과가 낫다. → `favicon.ico` 2168B, `layout.tsx`의 `sizes`는 `'32x32 48x48'`.
2. **`favicon.svg` 드리프트 감지 테스트 추가.** 자산이 커밋되는 구조라, SVG만 고치고
   `npm run favicons`를 안 돌리면 테스트가 전부 통과하면서 탭 아이콘과 SERP 아이콘이 조용히 갈라진다.
   `tests/app/favicon.test.ts`가 SVG의 sha256을 고정해 이를 잡는다.
   해시는 **CRLF→LF 정규화 후** 계산한다 — Windows `core.autocrlf` 때문에 정규화 없이는
   Linux CI에서 0 드리프트인데도 거짓 실패가 난다.
3. **`glyph-coverage.mjs`의 "최종 방어선은 git diff" 서술은 거짓이라 정정됨.**
   바이너리는 `git diff`가 "Binary files differ"만 출력한다. 실제 최종 방어선은 **사람이 PNG를 열어보는 것**이다.
4. **글자 커버리지 밴드(10~40%)의 출처는 1머신/1폰트**(Windows/Malgun Gothic)임을 코드에 명시.
   다른 폰트에서는 밴드를 넓혀야 할 수 있다.
