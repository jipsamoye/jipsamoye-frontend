...
- 알려진 한계(의도된 스코프 아웃, 구현하지 말 것): DM **방 목록**(`useDmRooms`)의 재연결 재조회는 핸드오프에서 제외됨. 열려 있는 방의 메시지만 재동기화한다. 오픈채팅(`/sub/chat/room`) 채널도 동일하게 재연결 재동기화 대상에서 제외 — 재연결 시 구독 복구만 되고 끊김 동안의 메시지 REST 재조회는 하지 않는다(의도된 결정).
