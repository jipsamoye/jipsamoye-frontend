...
```
Phase 1 (기반)
  [x] 1-1 폴더 구조 + API 클라이언트 + 타입 정의
  [x] 1-2 공통 레이아웃 (사이드바 + 헤더 + 반응형)
  [x] 1-3 다크모드 설정
  [x] 1-4 공통 컴포넌트 (Button, Modal, Card, Avatar, PostCard)

Phase 2 (핵심 페이지)
  [x] 2-1 메인 페이지 (배너 + 오늘의 멍냥 슬라이더 + 최신 게시글 무한스크롤)
  [x] 2-2 게시글 상세 (이미지 세로 나열 + 좋아요 + 댓글 CRUD + 작성자 다른 글)
  [x] 2-3 글쓰기 (이미지 드래그앤드롭 업로드 + 제목 + 내용)
  [x] 2-4 로그인 모달 (카카오/구글 버튼만 + 둘러보기 동작)

Phase 3 (부가 페이지)
  [x] 3-1 프로필 페이지 (커버 + 프로필 + 팔로우 + 게시글 목록)
  [x] 3-2 랭킹 페이지 (주간/월간 탭)
  [x] 3-3 검색 페이지 (검색창 + 결과 그리드)
  [x] 3-4 자유게시판 (준비 중 안내)
